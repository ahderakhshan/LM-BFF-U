...
class XLMRobertaForPromptFinetuning(XLMRobertaPreTrainedModel):
...
class AlbertForPromptFinetuning(AlbertPreTrainedModel):
    ...
    def forward(
        ...
        sequence_mask_output = sequence_output[
            torch.arange(sequence_output.size(0), device=sequence_output.device),
            mask_pos
        ]
        ...
        if self.return_full_softmax:
            if labels is not None:
                return torch.zeros(
                    1,
                    out=prediction_mask_scores.new()
                ), prediction_mask_scores

            return prediction_mask_scores

        # Return logits for each label word
        logits = []
