...
class FewShotDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, args, tokenizer, cache_dir=None, mode="train", use_demo=False):
        ...
        if self.use_demo and args.demo_filter:
            ...
            self.query_emb = np.load(os.path.join(args.data_dir, "{}.npy".format(split_name)))
            logger.info("Load embeddings (for demonstration filtering) from {}".format(os.path.join(args.data_dir, "{}.npy".format(split_name))))
            print(f"support_emb_length: {len(self.support_emb)}")
            print(f"support_examples_length: {len(self.support_examples)}")
            print(f"query_emb_length: {len(self.query_emb)}")
            print(f"query_examples_length: {len(self.query_examples)}")
        ...
        if mode != "train":
            ...
            for query_idx, context_indices, bootstrap_idx in self.example_idx:
                ...
                if use_query:
                    counter = 0
                    for s in supports:
                        if s.label == example.label:
                            supports[counter] = example
                            break
                        counter += 1
